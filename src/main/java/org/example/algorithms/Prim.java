...
public class Prim {
    public MSTResult computeMST(Graph graph) {
        ...
        while (!pq.isEmpty() && mstEdges.size() < n - 1) {
            ...
            int u = graph.indexOf(e.getFrom());
            int v = graph.indexOf(e.getTo());
        ...
        return new MSTResult("Prim", String.valueOf(graph.id()), mstEdges, totalCost, durationMs, operations);
    }
}
